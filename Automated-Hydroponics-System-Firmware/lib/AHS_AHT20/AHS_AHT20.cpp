 ...
 **************************************************************************/
void aht20_init(AHS_AHT20 *sensor)
{
    /* Snesor reading process start */
    i2c_start();
    i2c_setAddress(AHT20_SLAVE_ADDRESS, I2C_WRITE);
    i2c_write(TRIGGER_MEASUREMENT_CMD);
    i2c_write(TRIGGER_MEASUREMENT_PARAM_1);
    i2c_write(TRIGGER_MEASUREMENT_PARAM_2);
    i2c_stop();
    _delay_ms(80);
 ...
 **************************************************************************/
void aht20_update(AHS_AHT20 *sensor)
{
    i2c_start();
    i2c_setAddress(0x38, 1);

    /* Read byte 1 (State) */
    sensor->state = i2c_read();

    /* Read byte 2 (Humidity data [16:8]) */
    sensor->humidity_data_raw = (i2c_read());
